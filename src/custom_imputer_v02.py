...
class KZImputer(BaseEstimator, TransformerMixin):
    ...
    def _impute_series(self, series: pd.Series) -> pd.Series:
        s = series.copy()
        arr = s.to_numpy()  # Перевод в numpy для быстрого доступа

        mean_cache = {}  # Кэш для средних: ключ - (start, end)

        def cached_mean(start, end):
            # Кэшируем средние по срезам arr[start:end]
            if (start, end) not in mean_cache:
                # Важно исключить NaN из подсчёта среднего
        ...
        # Модифицированные версии функций импутации с использованием cached_mean и numpy
