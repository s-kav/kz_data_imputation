...
class KZImputer(BaseEstimator, TransformerMixin):
    ...
    def impute_gap(s: pd.Series, i: int, gap_size: int, pos: str) -> pd.Series:
        """
        Универсальная ускоренная импутация пропусков размером до 5 подряд.
        s — исходный Series
        i — индекс начала пропуска
        gap_size — длина пропуска (от 1 до 5)
        pos — позиция пропуска: 'left', 'right', 'middle'
        """
        if gap_size < 1 or gap_size > 5:
            raise ValueError("gap_size должен быть от 1 до 5")

        at = s.at  # быстрее, чем s.iloc[...] для одиночных обращений
